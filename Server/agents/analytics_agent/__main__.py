...
import json
from CFG import CHART_TYPES, MIN_N_CHARTS, MAX_N_CHARTS, MIN_N_TYPES, MAX_N_TYPES
...
class AnalyticsAgent:
    ...
    def _plan_visualizations(self, state: AnalyticsState, domain_info: dict):
        ...
        for chart_type, (min_charts, max_charts) in CHART_TYPES.items():
            if min_charts > 0 and max_charts < 99:
                if min_charts == max_charts:
                    analysis_constraints["rules"].append(
                        f"For {chart_type} charts, return exactly {min_charts} charts"
                    )
                else:
                    analysis_constraints["rules"].append(
                        f"For {chart_type} charts, return between {min_charts} and {max_charts} charts"
                    )
            elif min_charts > 0:
                analysis_constraints["rules"].append(
                    f"For {chart_type} charts, return at least {min_charts} charts"
                )
            elif max_charts < 99:
                analysis_constraints["rules"].append(
                    f"For {chart_type} charts, return at most {max_charts} charts"
                )
        ...
        if MIN_N_CHARTS < MAX_N_CHARTS:
            n_charts_str = f"between {MIN_N_CHARTS} and {MAX_N_CHARTS}"
        elif MIN_N_CHARTS == MAX_N_CHARTS:
            n_charts_str = f"exactly {MIN_N_CHARTS}"
        else:
            n_charts_str = f"at least {MIN_N_CHARTS}"
        ...
        - Use between {MIN_N_TYPES} and {MAX_N_TYPES} different chart types
        - Use only {n_charts_str} charts
        ...
        results = self._extract_json(response.choices[0].message.content)

        # bring all single_value charts to the front
        if "charts" in results:
            results["charts"].sort(key=lambda x: 0 if x["type"] == "single_value" else 1)
...
if __name__ == "__main__":
    ...
    with open("analytics_output.json", "w", encoding="utf-8") as f:
        json.dump(result_state.visualization_plan, f)
